...
void Port5_Init(void){
  ...
  // Use the PxSEL0 and PxSEL1 registers to enable alternate pin functions, if applicable
  // Use the port output register to put outputs to an initial state
    P5->SEL0 &= ~BIT3;       // GPIO mode
    P5->SEL1 &= ~BIT3;       // GPIO mode
    P5->DIR  |=  BIT3;       // Output
    P5->OUT  &= ~BIT3;       // Start off (LED not lit)
}
...
void Port7_Init(void){
  ...
  // Use the PxSEL0 and PxSEL1 registers to enable alternate pin functions, if applicable
  // Use the port output register to put outputs to an initial state
    P7->SEL0 = 0x00;  // All GPIO
    P7->SEL1 = 0x00;
    P7->DIR  = 0x00;  // All inputs
    P7->REN  = 0x00;  // No pull resistors
    P7->OUT  = 0x00;
}
...
void Port9_Init(void){
  ...
  // Use the PxSEL0 and PxSEL1 registers to enable alternate pin functions, if applicable
  // Use the port output register to put outputs to an initial state
    P9->SEL0 &= ~BIT2;       // GPIO mode
    P9->SEL1 &= ~BIT2;       // GPIO mode
    P9->DIR  |=  BIT2;       // Output
    P9->OUT  &= ~BIT2;       // Start off (LED not lit)
}
