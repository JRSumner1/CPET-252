...
typedef struct { uint8_t walkable; int8_t parent_x; int8_t parent_y;
                 uint16_t g,h,f; } Node_t;
...
static bool Astar_Path(Point s, Point g, Point *out, uint16_t *olen){
  ...
  openCount=0; openSet[openCount++]=s;

  while(openCount){
    int best=0;
    uint16_t bestf=grid[openSet[0].y][openSet[0].x].f,
             besth=grid[openSet[0].y][openSet[0].x].h;
    int i;
    ...
    if(cur.x==g.x && cur.y==g.y){
      ...
      out[n++]=s;
      int i;
      for(i=0;i<n/2;i++){ Point t=out[i]; out[i]=out[n-1-i]; out[n-1-i]=t; }
      *olen=n; return true;
    }
    ...
    for(k=0;k<4;k++){
      ...
      if(tent<grid[nb.y][nb.x].g){
        ...
        for(i=0;i<openCount;i++)
          if(openSet[i].x==nb.x&&openSet[i].y==nb.y){ in=true; break; }
...
#define TRY(HVAL)                                                             \
  do{                                                                         \
    ...
    if(nx>=0 && nx<GRID_COLS && ny>=0 && ny<GRID_ROWS                         \
      ...
      if(diff==1){ Motor_Right(0,7500); Clock_Delay1ms(TURN90_MS);}           \
      if(diff==3){ Motor_Left (7500,0); Clock_Delay1ms(TURN90_MS);}           \
      if(diff==2){ Motor_Right(0,7500); Clock_Delay1ms(TURN90_MS*2);}         \
      Motor_Stop(); curHeading=h2;                                            \
      Motor_Forward(7500,7500); Clock_Delay1ms(CELL_MS); Motor_Stop();        \
      curX=nx; curY=ny; visited[ny][nx]=true; DrawCell(nx,ny,1);              \
...
static void Robot_Explore(void){
  ...
  Point stack[GRID_ROWS*GRID_COLS]; int8_t sp=0;
  curX=curY=0; curHeading=1; visited[0][0]=true; stack[sp++] = (Point){0,0};
  ServoInit();
  SSD1306_ClearBuffer();
  SSD1306_DrawString(0,0,"Exploring",WHITE);
  DrawCell(0,0,1); SSD1306_DisplayBuffer();

  while(sp){
    Servo(SERVO_RIGHT); Clock_Delay1ms(40); uint16_t right_cm = distanceInCm();
    Servo(SERVO_LEFT);  Clock_Delay1ms(40); uint16_t left_cm  = distanceInCm();
    Servo(SERVO_CENTER);Clock_Delay1ms(40); uint16_t front_cm = distanceInCm();
    ...
    if(!moved){
      grid[curY][curX].walkable=0; DrawCell(curX,curY,2);
      sp--; if(sp==0) break;
      ...
      if(diff==3){ Motor_Left (7500,0); Clock_Delay1ms(TURN90_MS); }
      if(diff==2){ Motor_Right(0,7500); Clock_Delay1ms(TURN90_MS*2); }
      Motor_Stop(); curHeading=backH;
      Motor_Backward(7500,7500); Clock_Delay1ms(CELL_MS); Motor_Stop();
      curX=prev.x; curY=prev.y;
    }
    SSD1306_DisplayBuffer();
    if(curX==4 && curY==4) break;
  }
  Servo(SERVO_CENTER); Motor_Stop();
  SSD1306_ClearBuffer();
...
static void Robot_FollowPath(Point *path,uint16_t len){
  ...
  for(i=1;i<len;i++){
    ...
    Motor_Stop(); h=tgt;
    Motor_Forward(7500,7500); Clock_Delay1ms(CELL_MS); Motor_Stop();
    curX=path[i].x; curY=path[i].y; Clock_Delay1ms(80);
  }
  curHeading=h;
  SSD1306_ClearBuffer(); SSD1306_DrawString(0,0,"Arrived",WHITE); SSD1306_DisplayBuffer();
}

int main(void){
  DisableInterrupts();
  Clock_Init48MHz(); UART0_Init();
  Port1_Init(); Port2_Init(); Port3_Init(); Port5_Init();
  Port6_Init(); Port9_Init();
  TimerA0_Init(); TimerA2_Init(); TimerA3_Init();
  ServoInit(); GridInit(); Motor_Stop();
  ...
  EnableInterrupts();
  UART0_OutString("\n\rRSLK A* Robot\n\r");
  AP_Init(); AP_AddService(0xFFF0); BLE_Cmd=0;
  AP_AddCharacteristic(0xFFF1,1,&BLE_Cmd,0x02,0x08,"Command",0,&WriteByteCmd);
  AP_RegisterService(); AP_StartAdvertisementJacki();

  while(1){
    AP_BackgroundProcess();
    switch(BLE_Cmd){
      case CMD_EXPLORE: Robot_Explore(); BLE_Cmd=0; break;
      case CMD_ROUTE:{
        Point start={curX,curY}, goal={GRID_COLS-1,GRID_ROWS-1};
        Point path[OPEN_MAX]; uint16_t len=0;
        if(Astar_Path(start,goal,path,&len)){
          SSD1306_ClearBuffer();
          SSD1306_DrawString(0,0,"Routing",WHITE); SSD1306_DisplayBuffer();
          Robot_FollowPath(path,len);
        }else{
          SSD1306_ClearBuffer();
          SSD1306_DrawString(0,0,"No Path!",WHITE); SSD1306_DisplayBuffer();
        }
        BLE_Cmd=0; break;
      }
      default: Clock_Delay1ms(50);
    }
  }
}
